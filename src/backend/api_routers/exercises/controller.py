    ...
    async with conn.transaction():
        async with conn.cursor(
            ...
            await cursor.execute(
                query+filters+group_by+limit_filter, 
                (*params,)
            )
...
async def get_exercise(
    ...
    async with conn.cursor(
        ...
        await cursor.execute("""
            SELECT e.id, e.name, e.description, e.difficulty, 
            COALESCE(array_agg(
    ...
    try:
        async with conn.transaction():
            async with conn.cursor(
                ...
                for equipment_id in exercise.equipment:
                    await cursor.execute(
                ...
                for muscle_group_id in exercise.muscle_groups:
                    await cursor.execute(
